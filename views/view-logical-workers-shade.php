...
// Gathers states and fills
$all_status = Array();
$all_fill = Array();
$status = $SQL->query("select name, state, source, info from state where name not like '%.scarf.rl.ac.uk'");
...
foreach ($all_clusters as $name => $panels) {
    if (($panels !== "vm-nubes") and (array_key_exists($name, $personalities) === true)) {
        ...
        if (array_key_exists($name, $all_fill)) {
            $results[$group][$panel][$cluster][$name] = array_merge($results[$group][$panel][$cluster][$name], $all_fill[$name]);
            unset($all_fill[$name]);
        };
    }
}
...
foreach ($all_status as $hostname => $status) {
    if ($hostname !== '') {
        ...
        if (array_key_exists($hostname, $all_fill)) {
            $details = array_merge($details, $all_fill[$hostname]);
            unset($all_fill[$hostname]);
        };
